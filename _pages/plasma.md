...
* Les modèles dits cinétiques
  ...
  * Equation de Vlasov (sanns collisions): $ \frac {\partial f} {\partial t} + \vec V . \frac {\partial f} {\partial \vec r} + (q/m) (\vec E + \vec V \times \vec B) . \frac {\partial f} {\partial \vec V} = 0$
...
La MHD
======
 * Le modèle MHD (équations, obtention, hypothèses) est décrit en détails sur la page correspondante. Cependant, le passage d'un modèle bi-fluide à un modèle mono-fluide mérite ici quelques explications. Tout d'abord, la densité volumique du plasma $\rho (\vec r, t)$ représente essentiellement celles des ions, car la masse des électrons est négligeable. De même, la vitesse de plasma $\vec V (\vec r, t)$ représente celle des ions. La vitesse des électrons (qui n'est pas un paramètre du modèle MHD) peut alors s'en déduire à condition de connaître la densité de courant électrique $\vec J$, par $\vec V_e = \vec V - \vec J / ne$, avec $n$ la densité de particules (on suppose $n_e = n_i$ qui est l'hypothèse de quasineutralité de la MHD, et $e$ étant la charge électronique).
